...
RUN useradd -u 1000 -ms /bin/bash teste-laravel-9

# Make sure files/folders needed by the processes are accessable when they run under the nobody user
RUN chown -R teste-laravel-9.www-data /run && \
  chown -R teste-laravel-9.www-data /var/lib/nginx && \
  chown -R teste-laravel-9.www-data /var/log/nginx
